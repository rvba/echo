...
void callback_send_tag_group_create(void *user, VNodeID node_id, uint16 group_id, const char *name)
{
	...
	if(DEBUG_ENOUGH) printf("[enough] callback tag group create group %d name %s\n",group_id, name);
...
void callback_send_tag_create(void *user, VNodeID node_id, uint16 group_id, uint16 tag_id, const char *name, VNTagType type, VNTag *tag)
{
	...
	switch(type)
	{
		...
		case VN_TAG_UINT32 :
			...
			if(DEBUG_ENOUGH) printf("[enough] callback create tag name %s val %d\n", name, tag->vuint32);
		break;
		case VN_TAG_REAL64 :
	...
	if(DEBUG_ENOUGH) printf("[enough] callback send tag %d in group %d\n", tag_id, group_id);
	e_ns_update_node_version_data(node);
}
